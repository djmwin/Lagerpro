# LagerPro V37 - stabilisierte Firmenversion
...
def send_system_email(event_type, subject, body, recipients=None):
    ...
    if recipients is None:
        event_key = 'container_emails' if event_type == 'container_fertig' else ('reorder_emails' if event_type == 'nachbestellung' else 'notification_emails')
        raw=(os.environ.get('LAGERPRO_'+event_key.upper()) or cfg.get(event_key,'') or
             os.environ.get('LAGERPRO_NOTIFICATION_EMAILS') or cfg.get('notification_emails',''))
    ...
    host=(os.environ.get('SMTP_HOST') or cfg.get('smtp_host','')).strip()
    user=(os.environ.get('SMTP_USER') or cfg.get('smtp_user','')).strip()
    password=os.environ.get('SMTP_PASSWORD') or cfg.get('smtp_password','')
    sender=(os.environ.get('SMTP_SENDER') or cfg.get('smtp_sender','')).strip() or user
    ...
    try:
        ...
        use_ssl=(os.environ.get('SMTP_SSL') or cfg.get('smtp_ssl','0'))=='1'
        use_starttls=(os.environ.get('SMTP_STARTTLS') or cfg.get('smtp_starttls','1'))=='1'
        if use_ssl:
...
def check_reorders():
    ...
    for a in arts:
        ...
        if stock <= int(a['min_stock'] or 0):
            ...
            elif not open_alert['email_sent_at']:
                last_attempt=open_alert['last_email_attempt_at']
                retry_before=(datetime.now()-timedelta(minutes=30)).isoformat(timespec='seconds')
                if not last_attempt or last_attempt <= retry_before:
                    target=max(int(a['target_stock'] or 0),int(a['min_stock'] or 0)); suggested=max(0,target-stock)
    ...
    for alert_id,no,name,stock,min_s,target,suggested in notices:
        c=con(); c.execute("UPDATE reorder_alerts SET last_email_attempt_at=? WHERE id=?",(now_iso(),alert_id)); c.commit(); c.close()
...
def health():
    ...
    return {"ok":True,"database":True,"version":"V37"}
                ...
                if not slot:
                    ...
                    if rule in ("Nur Ebene 1","nur Ebene 1") and l!=1:
                        ...
                        neighbors=c.execute("""SELECT pallet_type FROM warehouse_slots
                            WHERE rack=? AND level=? AND position IN (?,?) AND load_carrier_id IS NOT NULL""",
                            (r,l,p-1,p+1)).fetchall()
                        this_euro=(article["pallet_type"]=="Euro")
                        if any((n["pallet_type"]=="Euro") != this_euro for n in neighbors if n["pallet_type"]):
                            msg="Nicht möglich: Euro und Einweg dürfen nicht direkt nebeneinander stehen."
                            c.close()
                            return page(f'<div class="kicker">MANUELLE DIREKTBUCHUNG</div><h1 class="page-title">Buchung abgelehnt</h1><div class="notice danger">{msg}</div><a class="button" href="/manual-booking?slot_code={slot_code}">Zurück</a>',"warehouse")
...
def security_gate():
    ...
    if last:
        try:
            cfg=get_settings()
            timeout=max(1,min(24,int(cfg.get('auto_logout','8') or 8)))
            if datetime.now()-datetime.fromisoformat(last)>timedelta(hours=timeout): session.clear(); return redirect('/login')
...
def settings_page():
    if not role_allowed('Admin'): return page('<div class="card">Nur Admin.</div>','more'),403
    c=con(); defaults={'company_name':'LagerPro','auto_logout':'8','four_eyes_threshold':'10','notification_emails':'','container_emails':'','reorder_emails':'','smtp_host':'','smtp_port':'587','smtp_user':'','smtp_password':'','smtp_sender':'','smtp_starttls':'1','smtp_ssl':'0'}
    if request.method=='POST':
        for k in defaults:
    ...
    <h2>E-Mail-Benachrichtigungen</h2><label>Standardempfänger (mehrere mit Komma)</label><input name="notification_emails" value="{vals['notification_emails']}" placeholder="lagerleitung@firma.de">
    <div class="row"><div><label>Container-fertig an</label><input name="container_emails" value="{vals['container_emails']}" placeholder="vorgesetzter@firma.de"></div><div><label>Nachbestellungen an</label><input name="reorder_emails" value="{vals['reorder_emails']}" placeholder="einkauf@firma.de"></div></div>
    ...
    <label>SMTP-Passwort / App-Passwort</label><input type="password" name="smtp_password" value="" placeholder="Leer lassen, um das gespeicherte Passwort beizubehalten">
