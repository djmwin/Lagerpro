# LagerPro V38 - PostgreSQL, Railway-Persistenz und Produktionsschutz
from flask import Flask, request, redirect, session
import sqlite3, os, math, json, shutil, secrets, smtplib, ssl, re, hmac
...
class CompatRow:
    """DB row compatible with the sqlite Row access used by LagerPro."""
...
class PgResult:
    ...
    def _wrap(self, row):
        return None if row is None else CompatRow(row, self._result.keys())
...
class PgConnection:
    def __init__(self):
        ...
        self.engine = _postgres_engine(create_engine, url)
        self.conn = self.engine.connect()
        self.tx = self.conn.begin()

    @staticmethod
    def _params(sql, params):
    ...
    def execute(self, sql, params=()):
        ...
        sql = sql.strip()
        sql = sql.replace(
            "GROUP_CONCAT(rack||'-'||level||'-'||position)",
            "STRING_AGG(CAST(rack AS TEXT)||'-'||CAST(level AS TEXT)||'-'||CAST(position AS TEXT), ',')"
        )
        sql = re.sub(r"^INSERT\s+OR\s+IGNORE\s+INTO", "INSERT INTO", sql, flags=re.I)
        ignore = bool(re.match(r"^INSERT\s+INTO\s+warehouse_slots", sql, flags=re.I))
        wants_id = bool(re.match(r"^INSERT\s+INTO\s+(reorder_alerts|load_carriers)\b", sql, flags=re.I))
        if ignore and "ON CONFLICT" not in sql.upper():
            sql += " ON CONFLICT DO NOTHING"
        if wants_id and "RETURNING" not in sql.upper():
            sql += " RETURNING id"
        converted, bound = self._params(sql, params)
        result = self.conn.execute(text(converted), bound)
        lastrowid = None
        if wants_id:
            row = result.fetchone()
            lastrowid = row[0] if row else None
        return PgResult(result, lastrowid)
...
_PG_ENGINE = None
def _postgres_engine(create_engine, url):
    global _PG_ENGINE
    if _PG_ENGINE is None:
        _PG_ENGINE = create_engine(url, pool_pre_ping=True, pool_recycle=300)
    return _PG_ENGINE
...
def send_system_email(event_type, subject, body, recipients=None):
    ...
    if recipients is None:
        raw=os.environ.get('NOTIFICATION_EMAILS') or cfg.get('notification_emails','')
    ...
    host=(os.environ.get('SMTP_HOST') or cfg.get('smtp_host','')).strip(); user=(os.environ.get('SMTP_USER') or cfg.get('smtp_user','')).strip(); password=os.environ.get('SMTP_PASSWORD') or cfg.get('smtp_password',''); sender=(os.environ.get('SMTP_SENDER') or cfg.get('smtp_sender','')).strip() or user
    try: port=int(os.environ.get('SMTP_PORT') or cfg.get('smtp_port','587') or 587)
    except: port=587
    ...
    try:
        ...
        use_ssl=os.environ.get('SMTP_SSL',cfg.get('smtp_ssl','0'))=='1'
        use_starttls=os.environ.get('SMTP_STARTTLS',cfg.get('smtp_starttls','1'))=='1'
        if use_ssl:
            ...
            with smtplib.SMTP(host,port,timeout=15) as srv:
                srv.ehlo()
                if use_starttls: srv.starttls(context=ssl.create_default_context()); srv.ehlo()
                if user: srv.login(user,password)
                srv.send_message(msg)
...
def health():
    ...
    return {"ok":True,"database":True,"database_backend":"postgresql" if IS_POSTGRES else "sqlite","version":"V38"}
...
def security_headers(response):
    ...
    response.headers['Content-Security-Policy']="default-src 'self' data:; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; connect-src 'self';"
...
def security_gate():
    if request.endpoint in {'login','setup_admin','health','manifest','service_worker','static'}:
        if request.method == 'POST':
            supplied=request.form.get('csrf_token',''); expected=session.get('csrf_token','')
            if not expected or not hmac.compare_digest(supplied, expected):
                return page('<div class="card"><h2>Sicherheitsprüfung fehlgeschlagen</h2><p>Bitte Seite neu laden und erneut versuchen.</p></div>','more'),400
        return
...
def backup_page():
    ...
    if request.method=='POST':
        if IS_POSTGRES:
            msg='PostgreSQL wird über Railway gesichert. Aktiviere dort tägliche Backups und Point-in-Time-Recovery.'
        else:
...
def finish_container(cid):
    ...
    if cont['completion_email_sent_at']:
        c.close()
        return redirect(f'/container/{cid}')
    finished=now_iso()
    claim='wird_gesendet:'+finished
    claimed=c.execute("UPDATE containers SET status='erledigt',unload_finished_at=?,completion_email_sent_at=? WHERE id=? AND completion_email_sent_at IS NULL",(finished,claim,cid))
    c.commit(); c.close()
    if claimed.rowcount != 1:
        return redirect(f'/container/{cid}')
    who=session.get('username') or 'Unbekannt'; gate=cont['gate_no'] or '–'
    sent=send_system_email('container_fertig',f"Container fertig: {cont['container_no']} · Tor {gate}",f"Container {cont['container_no']} wurde fertig gebucht.\nTor: {gate}\nFertig am: {finished}\nGebucht von: {who}\n\nLagerPro")
    c=con()
    if sent: c.execute("UPDATE containers SET completion_email_sent_at=? WHERE id=? AND completion_email_sent_at=?",(now_iso(),cid,claim))
    else: c.execute("UPDATE containers SET completion_email_sent_at=NULL WHERE id=? AND completion_email_sent_at=?",(cid,claim))
    c.commit(); c.close()
...
def settings_page():
    ...
    <h2>E-Mail-Benachrichtigungen</h2><div class="notice">Für Railway sollten SMTP-Passwort und Zugangsdaten als Umgebungsvariablen gespeichert werden. Dort hinterlegte Werte haben Vorrang.</div><label>Empfänger (Vorgesetzter / Einkauf; mehrere mit Komma)</label><input name="notification_emails" value="{vals['notification_emails']}" placeholder="vorgesetzter@firma.de, einkauf@firma.de">
    ...
    <label>SMTP-Passwort / App-Passwort</label><input type="password" name="smtp_password" value="" placeholder="Leer lassen, um das vorhandene Passwort beizubehalten">
